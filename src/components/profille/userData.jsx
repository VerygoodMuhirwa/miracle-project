...
import { userActions } from "../../store/slices/userCredentials";
...
const UserData = () => {
  ...
  const countryOne=useSelector(state=>state.logged.userData.profile.country)
  let countryOriginal
  ...
  const [emailValue, setEmailNameValue] = useState(email);
  const [phoneNumberValue, setPhoneNameValue] = useState(phoneNumber);
  const [timeValue, setTimeValue] = useState("");
  const [openToCollabrateValue, setOpenToCollabrateValue] =
    useState(openToCollabrate);
  const [flagValue, setFlagValue] = useState(
    userData?.countryInfo?.flag
  );
...
console.log(countryOriginal);
  ...
  const handleSubmition = async (e) => {
    ...
    const tutorialSkills = state.userCredentials.skills;
